...
To build the application, run the Maven `install` and `package` goals from the command line in the `finish` directory:
...
Next, use the provided script to start the application in Docker containers. The script creates a network for the containers to communicate with each other. It also creates containers for Kafka and Zookeeper.
...
Start dev mode for the system service: from `finish/system`, run `mvn liberty:devc`

Start dev mode for the inventory service: from `finish/inventory`, run `mvn liberty:devc`
